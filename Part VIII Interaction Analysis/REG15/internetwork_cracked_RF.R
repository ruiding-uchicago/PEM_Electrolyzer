...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD15')
...
data_PC_train=read.csv("REG15_train.csv",head=T)
data_PC_test=read.csv("REG15_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CD15 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CD15 ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
x_y<-data.frame(Source=c(rep('CD15',13)),
...
##################################################
total_inter_counts<-c(4024, 3369, 3465, 2785, 2388, 2873, 3720, 2956, 3220, 3291, 3161, 3053, 2277)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CD15','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CD15','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CD15.csv')
#NDS<-read_csv('test/nodes----CD15.csv')
##################################################
EDG<-edges
NDS<-nodes
