...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD16')
...
data_PC_train=read.csv("REG16_train.csv",head=T)
data_PC_test=read.csv("REG16_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CD16 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CD16 ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
x_y<-data.frame(Source=c(rep('CD16',13)),
...
##################################################
total_inter_counts<-c(8455, 6662, 6425, 5989, 5036, 5965, 7706, 6234, 6991, 6612, 6793, 7382, 4732)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CD16','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CD16','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CD16.csv')
#NDS<-read_csv('test/nodes----CD16.csv')
##################################################
EDG<-edges
NDS<-nodes
