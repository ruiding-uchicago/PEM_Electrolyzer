...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD18')
...
data_PC_train=read.csv("REG18_train.csv",head=T)
data_PC_test=read.csv("REG18_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CD18 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CD18 ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
x_y<-data.frame(Source=c(rep('CD18',13)),
...
##################################################
total_inter_counts<-c(3965, 3135, 2795, 2932, 2169, 3102, 3762, 2878, 3527, 3117, 3377, 3715, 2588)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CD18','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CD18','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CD18.csv')
#NDS<-read_csv('test/nodes----CD18.csv')
##################################################
EDG<-edges
NDS<-nodes
