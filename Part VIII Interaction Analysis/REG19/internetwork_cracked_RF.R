...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD19')
...
data_PC_train=read.csv("REG19_train.csv",head=T)
data_PC_test=read.csv("REG19_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CD19 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CD19 ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
x_y<-data.frame(Source=c(rep('CD19',13)),
...
##################################################
total_inter_counts<-c(1941, 1604, 1483, 1351, 1111, 1584, 1925, 1420, 1757, 1634, 1806, 1861, 1293)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CD19','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CD19','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CD19.csv')
#NDS<-read_csv('test/nodes----CD19.csv')
##################################################
EDG<-edges
NDS<-nodes
