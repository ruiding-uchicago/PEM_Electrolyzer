...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD20')
...
data_PC_train=read.csv("REG20_train.csv",head=T)
data_PC_test=read.csv("REG20_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CD20 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CD20 ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
x_y<-data.frame(Source=c(rep('CD20',13)),
...
##################################################
total_inter_counts<-c(7754, 6353, 5853, 5522, 4250, 6210, 7614, 5827, 7132, 6219, 6653, 7397, 5610)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CD20','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CD20','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CD20.csv')
#NDS<-read_csv('test/nodes----CD20.csv')
##################################################
EDG<-edges
NDS<-nodes
