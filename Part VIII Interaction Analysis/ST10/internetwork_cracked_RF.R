...
##################################################
set.seed(6)
# <- randomForest(CL_10 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CL_10 ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-16],'CL_10'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',6),rep('#ffc177',4),rep('#000000',2),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-16],'CL_10'))
...
x_y<-data.frame(Source=c(rep('CL_10',15)),
...
##################################################
total_inter_counts<-c(180, 376, 339, 312, 316, 368, 374, 337, 535, 418, 370, 202, 172, 437, 540)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CL_10','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CL_10','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CL_10.csv')
#NDS<-read_csv('test/nodes----CL_10.csv')
##################################################
EDG<-edges
NDS<-nodes
