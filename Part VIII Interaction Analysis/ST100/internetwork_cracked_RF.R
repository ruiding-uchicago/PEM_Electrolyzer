...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD',	'ST','CL_100')
...
data_PC_train=read.csv("ST100_train.csv",head=T)
data_PC_test=read.csv("ST100_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CL_100 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CL_100 ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-16],'CL_100'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',6),rep('#ffc177',4),rep('#000000',2),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-16],'CL_100'))
...
x_y<-data.frame(Source=c(rep('CL_100',15)),
...
##################################################
total_inter_counts<-c(238, 324, 428, 244, 178, 357, 432, 355, 363, 424, 288, 271, 118, 270, 454)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CL_100','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CL_100','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CL_100.csv')
#NDS<-read_csv('test/nodes----CL_100.csv')
##################################################
EDG<-edges
NDS<-nodes
