...
colindex<-c('OT',	'FR',	'AA',	'IR',	'RU',	'O',	'ICA',	'PT',	'ICC',	'AL',	'CL',	'MT',	'EW',	'CD',	'ST','CL_50')
...
data_PC_train=read.csv("ST50_train.csv",head=T)
data_PC_test=read.csv("ST50_test.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(6)
# <- randomForest(CL_50 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(CL_50 ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-16],'CL_50'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',6),rep('#ffc177',4),rep('#000000',2),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-16],'CL_50'))
...
x_y<-data.frame(Source=c(rep('CL_50',15)),
...
##################################################
total_inter_counts<-c(859, 1497, 1802, 791, 687, 1518, 1420, 1147, 1535, 1569, 1011, 932, 738, 1303, 1813)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','CL_50','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','CL_50','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----CL_50.csv')
#NDS<-read_csv('test/nodes----CL_50.csv')
##################################################
EDG<-edges
NDS<-nodes
